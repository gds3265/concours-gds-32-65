...
v0.8.1 : espace partenaires, accès par concours, vérification facultative et validation finale simplifiée.

NOUVEAUTÉS v0.8.1 — MODE PARTENAIRE AUTOMATIQUE
...
NOUVEAUTÉS v0.8.1 — IMPORT CSV INTELLIGENT
...
NOUVEAUTÉS v0.8.1 — QUALIFICATIONS EN MASSE
...
NOUVEAUTÉS v0.8.1 — CORRECTION IMPORT QUALIFICATIONS
...
NOUVEAUTÉS v0.8.1 — CORRECTION RAPPROCHEMENT QUALIFICATIONS
...
NOUVEAUTÉS v0.8.1 — CORRECTION FICHIERS QUALIFICATIONS EXCEL
...
NOUVEAUTÉS v0.8.1 — SUIVI GESTIONNAIRE DES PARTENAIRES
...
NOUVEAUTÉS v0.8.1 — PARTENAIRES UTILISABLES DE BOUT EN BOUT
...
NOUVEAUTÉS v0.8.1 — PORTAIL PARTENAIRE : 2 MODES DISTINCTS
...
NOUVEAUTÉS v0.8.1 — FINALISATION PORTAIL PARTENAIRE
...
NOUVEAUTÉS v0.8.1 — CONTRÔLE À L'ENTRÉE DU CONCOURS
...
NOUVEAUTÉS v0.8.1 — AFFICHAGE GLOBAL PAR DÉFAUT
...
NOUVEAUTÉS v0.8.1 — LISIBILITÉ PAR CHEPTEL + LISTES DE CONTRÔLE
...
NOUVEAUTÉS v0.8.1 — RENOUVELLEMENT AUTOMATIQUE DE SESSION SUPABASE
...
NOUVEAUTÉS v0.8.1 — ANALYSES + ZOOM FIN
- Suppression de la date de prélèvement du cadre global Analyses.
- Les changements de statut dans Analyses ne modifient plus les dates déjà enregistrées.
- Les boutons Tout conforme / Tout en cours labo / Tout en attente ne touchent plus aux dates.
- Ajout d'un zoom propre à l'application dans l'en-tête.
- Réglage de 45 % à 110 % par pas de 1 %.
- Boutons - et +, curseur et affichage du pourcentage.
- Cliquer sur le pourcentage remet le zoom à 100 %.
- Le zoom choisi est mémorisé sur l'appareil.
- Conseil : remettre le zoom du navigateur à 100 %, puis choisir par exemple 58 %, 60 %, 62 % ou 63 % dans l'application.
- Aucun SQL supplémentaire.

NOUVEAUTÉS v0.8.1 — CORRECTION CHARGEMENT CERTIFICATS
- Correction de l'erreur "Cannot access 'certRecapSort' before initialization".
- La variable de tri du récapitulatif Certificats est maintenant initialisée
  avec les variables globales avant le premier chargement/rendu.
- Suppression de l'ancienne déclaration tardive.
- Aucun SQL supplémentaire.

NOUVEAUTÉS v0.8.1 — FORÇAGE MISE À JOUR / ANTI-CACHE
- La correction certRecapSort de v0.7.8 est conservée.
- Ajout d'un build marker 079 visible dans l'en-tête.
- Navigation HTML forcée en réseau/no-store lorsque la connexion est disponible.
- Service worker forcé à vérifier sa mise à jour sans utiliser le cache HTTP.
- Suppression automatique des anciens caches lors de l'activation.
- Les autres ressources sont revalidées avant utilisation.
- Le cache n'est utilisé comme secours qu'en cas de vraie absence réseau.
- Aucun SQL supplémentaire.

NOUVEAUTÉS v0.8.1 — CORRECTION DÉMARRAGE COMPLET
- Correction de "Cannot access 'fastCurrentHerd' before initialization".
- fastCurrentHerd et fastCurrentFarmer sont désormais initialisés avec les autres variables globales avant le premier renderAll().
- Vérification du fichier complet : aucune autre déclaration globale 'let' utilisée par renderAll ne reste placée après loadAll().
- Le message de session expirée devient non bloquant : retour à l'écran de connexion avec un simple message.
- Nouveau marqueur visible 080 et nouveau cache forcé.
- Aucun SQL supplémentaire.

NOUVEAUTÉS v0.8.1 — NAVIGATION SÉCURISÉE
- Correction du blocage où aucun onglet ne s'ouvrait.
- Navigation désormais gérée par un seul gestionnaire délégué sur la barre d'onglets.
- Le changement d'onglet est effectué avant tout rendu métier.
- Un onglet ne relance plus tout renderAll() : seul le module concerné est rafraîchi.
- Une erreur dans un module ne peut donc plus empêcher l'ouverture des autres onglets.
- Règles CSS renforcées pour l'affichage de .section.active.
- Nouveau marqueur visible 081 et nouveau cache forcé.
- Aucun SQL supplémentaire.
